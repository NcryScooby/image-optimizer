...
func TestBuildPath_FullParams(t *testing.T) {
	...
	got := BuildPath("originals/abc.jpg", p)

	want := "/insecure/rs:fill:300:200/g:ce/q:80/plain/local:///originals/abc.jpg@avif"
	if got != want {
...
func TestBuildPath_FitMapping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.fit, func(t *testing.T) {
			got := BuildPath("originals/x.png", transform.Params{
...
func TestBuildPath_CropMapping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.crop, func(t *testing.T) {
			got := BuildPath("originals/x.webp", transform.Params{
...
func TestBuildPath_NilDimensions(t *testing.T) {
	got := BuildPath("originals/id.jpeg", transform.Params{Q: 80})
	want := "/insecure/rs:fit:0:0/g:ce/q:80/plain/local:///originals/id.jpeg@avif"
	if got != want {
		t.Fatalf("BuildPath() = %q, want %q", got, want)
	}
}
...
func TestBuildPath_StripsLeadingSlash(t *testing.T) {
	got := BuildPath("/originals/abc.jpg", transform.Params{Q: 50, Fit: "contain"})
	if !strings.Contains(got, "local:///originals/abc.jpg@avif") {
		t.Fatalf("unexpected local source in %q", got)
	}
	if strings.Contains(got, "local:////") {
		t.Fatalf("double slash in local source: %q", got)
	}
}

func TestBuildPath_AlwaysAVIF(t *testing.T) {
	got := BuildPath("originals/a.png", transform.Params{Q: 1})
